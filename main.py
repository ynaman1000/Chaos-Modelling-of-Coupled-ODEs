...
stps = [0.00001, 0.0005, 0.001]          # incremental time step
ti = 0.0                        # initial time
tf = 5.0                        # final time
...
I, J, K, L, M = 1, 1, len(stps)-1, 1, len(mss)
nrws = M                        # number of rows of subplots
...
if type(axs) is np.ndarray:
	...
	# 	uncomment next 3 lines if commented and set all the functions' arguments
        ...
        states = solve(stps[len(stps)-1], ti, tf, slvrs[len(slvrs)-1], ys_inits[0], mss[i], lss[0])
        plt_fun(axs[i], states, lss[0], mss[i], "M: "+str(mss[i])+" | "+str(stps[len(stps)-1])+" | "+slvrs[len(slvrs)-1]["solver"], 'k')
